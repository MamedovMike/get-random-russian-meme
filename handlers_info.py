...
HELP_COMMAND_TEXT = HELP_COMMAND_TEXT = """Как пользоваться ботом:

Используй кнопки снизу экрана - "Получить Мем" пришлёт случайную картинку из базы, "Отправить Мем" подскажет, что делать дальше.

Прислать свой мем можно и просто фото, без нажатия кнопки - он попадёт на модерацию, и после одобрения окажется в общей базе.
...
Я обожаю <b><u>Русские Мемы</u></b> - постоянно делюсь ими с друзьями, кидаю в чаты и просто пересматриваю свою галерею. В какой-то момент понял, что единого места, где собраны именно русские мемы, не существует - и решил сделать такое место сам.
